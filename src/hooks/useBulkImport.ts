...
export function useBulkImport() {
  ...
  const processImport = useCallback(async (
    ...
    try {
      ...
      // Now process outdoors (code is auto-generated by DB trigger)
      for (let i = 0; i < outdoors.length; i++) {
        ...
        try {
          ...
          if (options.ignoreDuplicates) {
            ...
            if (existing) {
              results.push({ sucesso: true, tipo: 'outdoor', nome: `Outdoor existente`, linha: lineNum });
              processed++;
              setProgress(Math.round((processed / total) * 100));
              continue;
            }
          ...
          const { data: newOutdoor, error } = await supabase
            .from('outdoors')
            .insert({
          ...
          outdoorsCreated++;
          results.push({ sucesso: true, tipo: 'outdoor', nome: newOutdoor?.code || 'Outdoor', linha: lineNum });
        } catch (error: any) {
          results.push({ 
            sucesso: false, 
            tipo: 'outdoor', 
